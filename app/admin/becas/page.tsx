...
export default async function AdminBecasPage() {
  ...
  return (
    <div className="page-enter space-y-6">
      <PrivacyToggle headerActions={<SendWelcomeButton />}>
                ...
                <tbody>
                  {poolScholarships.map((s) => (
                    <tr key={s.id}>
                      ...
                      <td className="px-3 py-2"><span className="pv">{s.grantor.email}</span></td>
              ...
              <tbody>
                {allScholarships.map((s) => (
                  <tr key={s.id}>
                    <td className="px-3 py-2 font-mono">{s.scholarship_code}</td>
                    <td className="px-3 py-2"><span className="pv">{s.grantor.email}</span></td>
                    <td className="px-3 py-2"><span className="pv">{s.recipient?.email ?? s.recipient_email ?? "—"}</span></td>
              ...
              <div className="mt-3 overflow-x-auto">
                <table className="liquid-table w-full text-left text-xs">
                  ...
                  <tbody>
                    {reviewedApps.map((a) => (
                      <tr key={a.id}>
                        <td className="px-3 py-2"><span className="pv">{a.user.email}</span></td>
                        <td className="max-w-xs truncate px-3 py-2 text-slate-400"><span className="pv">{a.reason}</span></td>
                        <td className="px-3 py-2">
                          <span className={`rounded-full px-1.5 py-0.5 text-[10px] font-medium ${
